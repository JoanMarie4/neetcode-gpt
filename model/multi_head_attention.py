...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        outputs = []
        for h in self.attention_heads:
            outputs.append(h(embedded))
        concat_result = torch.cat(outputs, dim=2)
        return torch.round(self.output_proj(concat_result), decimals=4)
